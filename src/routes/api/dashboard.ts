...
function configuredProfile(email: string): { role: ViewerRole; employeeId: string | null; employeeName: string | null } | null {
  ...
  try {
    ...
    if (!role) return null;
    return { role, employeeId: typeof profile.employeeId === "string" ? profile.employeeId : null, employeeName: typeof profile.employeeName === "string" ? profile.employeeName : null };
  } catch {
    return null;
  }
}
  ...
  server: {
    ...
    handlers: {
      GET: async ({ context }) => {
        ...
        if (role === "manager") {
          const own = data.employees.find((employee) =>
        ...
        if (role === "employee") {
          const employee = data.employees.find((candidate) =>
            (employeeId && candidate.id === employeeId) ||
            (employeeName && candidate.name.toLowerCase() === employeeName.toLowerCase()),
          ) ?? null;
          return Response.json({ viewerRole: "employee", month: data.month, employee } satisfies DashboardView);
        }
